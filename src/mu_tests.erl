...
insert_questionnaire1() ->
  ...
  QuestionMap= [#{<<"answers">> => [#{<<"conditions">> => [],
   ...
   <<"id">> => 1,
   <<"image">> => <<>>,
   <<"question">> => <<"»Menim, da ne morem veliko narediti za izboljšanje svojih umskih sposobnosti.«">>},
 #{<<"answers">> => [#{<<"conditions">> => [],
   ...
   <<"id">> => 2,
   <<"image">> => <<>>,
   <<"question">> => <<"»Verjamem, da lahko s trudom precej izboljšam svoje umske sposobnosti.«">>},
 #{<<"answers">> => [#{<<"conditions">> => [#{<<"condition">> => [#{<<"answer">> => 1,<<"id">> => 1},
          #{<<"op">> => <<"and">>},
          #{<<"answer">> => 1,<<"id">> => 2},
          #{<<"op">> => <<>>}],
         <<"nextQuestion">> => 1},
       #{<<"condition">> => [#{<<"answer">> => 2,<<"id">> => 1},#{<<"op">> => <<>>}],
         <<"nextQuestion">> => 3}],
 ...
 #{<<"answers">> => [#{<<"conditions">> => [],
   ...
   <<"id">> => 4,
   <<"image">> => <<>>,
   <<"question">> => <<"»Menim, da imam ne glede na svojo trenutno stopnjo umskih sposobnosti zmožnost, da jo precej izboljšam.«">>},
 #{<<"answers">> => [],
   <<"answers_type">> => <<"selectOne">>,
   <<"id">> => 5,
   <<"image">> => <<>>,
 ...
 #{<<"answers">> => [],
   ...
   <<"id">> => 6,
   <<"image">> => <<>>,
   <<"question">> => <<"Super, spoznal si dve miselni naravnanosti. Razmisli o njih še v situacijah delovnega konteksta.">>},
 #{<<"answers">> => [#{<<"conditions">> => [],
 ...
 #{<<"answers">> => [#{<<"conditions">> => [],
      ...
      <<"id">> => 1,
      <<"value">> => <<"Razmišljal bi, da ne znam delati v skupini in da nimam dovolj dobrih socialnih spretnosti, katerih ne morem izboljšati in zato nikoli ne bom izbran.">>,
      <<"weight">> => <<"1">>},
    #{<<"conditions">> => [],
      <<"defaultNextQuestion">> => <<"9">>,
      <<"id">> => 2,
      <<"value">> => <<"Razmišljal bi, da se nisem dovolj uril za delo v skupini. Pristopil bi do sodelavcev in nadrejenih ter se pozanimal, kako izboljšati kompetence dela v skupinah.">>,
      <<"weight">> => <<"2">>}],
   <<"answers_type">> => <<"selectOne">>,
   <<"id">> => 8,
   <<"image">> => <<>>,
